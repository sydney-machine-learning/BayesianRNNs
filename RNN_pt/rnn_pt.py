...
class Network:
    ...
    def __init__(self,LearnRate, Topo, train_x,train_y,test_x,test_y):
        ...
        self.B1 = np.random.randn(1,self.Top[1])      # bias first layer
        self.W2 = np.random.randn(self.Top[1] , self.Top[2])
        self.B2 = np.random.randn(1,self.Top[2])    # bias second layer
    ...
    def ForwardPass(self, sample,slide):
        ...
        #print(sample_time,' is sample time')
        layer = 0 
        weightsum = 0.0
        StateWeightSum = 0.0
        forwardout=0.0
        ...
        for y in range(0, self.Top[1]):
            ...
            #print(weightsum,' weightsum',StateWeightSum,' Stateweight',self.B1[0][y],'b1[y]')
            forwardout = (weightsum + StateWeightSum) - self.B1[0][y]
            # if self.OutputSlideL1 == []:
            #     print('blah2')
            #     quit()
            #     self.OutputSlideL1 = np.zeros((len(sample)+1,self.Top[1]))
            #print(forwardout,' is forwardout')
        ...
        for y in range(0, self.Top[layer+1]):
            for x in range(0, self.Top[layer]):
                ...
                forwardout = (weightsum - self.B2[0][y])
    ...
    # compute gradients for each layer (output and hidden layer)
        ...
        for y in range(0,self.Top[2]):
            ...
            self.B2[0][y] += topbias - (topbias * weightdecay)
            topbias=0.0
        ...
        for y in range(0,self.Top[1]):
            ...
            self.B1[0][y] += tmp1 - (tmp1 * weightdecay)
    ...
    # hoping that it works properly for the time being
    # need to modify appropriately,
    def encode(self):
        w1 = self.W1.ravel()
        ...
        #print(w1.T.shape,w2.T.shape,self.B1.T.shape,self.B2.T.shape,w3.T.shape)
    ...
    def langevin_gradient(self,train_x,train_y,w):
        self.decode(w) 
        ...
        for i,sample in enumerate(train_x):
            ...
            for slide in range(len(sample),0,-1):
                self.BackwardPass(sample,self.learn_rate,train_y[i-1],slide)
        w1 = self.encode()
        return w1
...
class ptReplica(multiprocessing.Process):
    ...
    def __init__(self, use_langevin_gradients, learn_rate,  w,  minlim_param, maxlim_param, samples,trainx,trainy,testx,testy, topology, burn_in, temperature, swap_interval, langevin_prob, path, parameter_queue, main_process,event ):
    ...
    def run(self):
        ...
        step_eta = 0.2
        #Declare RNN
        rnn = Network(learn_rate, self.topology, self.train_x,self.train_y, self.test_x,self.test_y)
...
class ParallelTempering:
    ...
    def __init__(self,  use_langevin_gradients, learn_rate, train_x,train_y,test_x,test_y, topology, num_chains, maxtemp, NumSample, swap_interval, langevin_prob, path):
        #FNN Chain variables
        self.train_x = train_x
        self.train_y = train_y
        self.test_x = test_x
    ...
    def initialize_chains(self,  burn_in):
        ...
        for i in range(0, self.num_chains):
            ...
            self.chains.append(ptReplica(self.use_langevin_gradients, self.learn_rate, w,  self.minlim_param, self.maxlim_param, self.NumSamples,self.train_x,self.train_y,self.test_x,self.test_y,self.topology,self.burn_in,self.temperatures[i],self.swap_interval, self.langevin_prob, self.path,self.parameter_queue[i],self.wait_chain[i],self.event[i]))
...
def main():
    ...
    for i in range(1, 2) : 
        ...
        pt = ParallelTempering( use_langevin_gradients,  learn_rate,  train_x,train_y,test_x,test_y, topology, num_chains, maxtemp, NumSample, swap_interval, langevin_prob, path)
