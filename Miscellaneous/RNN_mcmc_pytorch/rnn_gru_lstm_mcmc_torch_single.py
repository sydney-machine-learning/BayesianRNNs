...
RNN,GRU,LSTM with mcmc in torch
...
parser=argparse.ArgumentParser(description='RNN - Gru and LSTM MCMC implementation in torch')
...
# just to debug, put f() wherever you want and it will stop this application
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        outmain=torch.zeros((x.shape[0],self.topo[2]))
...
class MCMC:
    ...
    def likelihood_func(self, neuralnet, x,y, w, tausq):
        ...
        #fx = np.array(fx) #this might vary with different inputs
        y = torch.FloatTensor(y).view(fx.shape)
        rmse = self.rmse(fx, y)
    ...
    def sampler(self):
        ...
        # no need for w_size
        w_size = sum(p.numel() for p in neuralnet.parameters())
        ...
        pred_test = np.array(pred_test).reshape(y_test.shape)
        pred_train = np.array(pred_train).reshape(y_train.shape)
        ...
        pred_test = np.array(pred_test).reshape(y_test.shape)
        pred_train = np.array(pred_train).reshape(y_train.shape)
        plt.plot(x_train, np.array(y_train))
        ...
        for i in range(samples - 1):
            ...
            if u < mh_prob:
                print(i,rmsetrain,rmsetest,'accepted and rmse train & test')
                naccept += 1
...
def main():
        ...
        fx_low = fx_low.reshape(-1)
        fx_high = fx_high.reshape(-1)
        ...
        plt.clf()
        # -----------------------------------------
        fx_low_tr = fx_low_tr.reshape(-1)
        fx_high_tr = fx_high_tr.reshape(-1)
