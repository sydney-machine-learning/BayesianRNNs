...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul 11 15:44:45 2019
...
class Model(nn.Module):
    ...
    def __init__(self, topo,lrate,batch_size,rnn_net = 'RNN'):
        ...
        if rnn_net == 'GRU':
            ...
            #self.rnn1 = nn.LSTM(input_size = topo[1],hidden_size = topo[1])
        else:
            self.hidden = torch.ones(self.n_layers, self.batch_size, self.hidden_dim)
        ...
        self.fc1 = nn.Linear(topo[1],topo[1])
...
#        return 1/(1+torch.exp(-z))
    ...
    def forward(self, data):
        ...
        fx = torch.zeros((len(data),self.batch_size))
        for i,sample in enumerate(data,0):
            ...
            #print(hidden[0].shape,'h shape')
            #f()
            out, h1 = self.rnn(inputs, hidden)
            #out, h1 = self.rnn1(out.view(self.topo[0],self.batch_size,self.topo[1]),copy.deepcopy(self.hidden))
            out = self.fc1(out[-1])
            out = self.relu(out)
            out = self.fc(out)
            #out = self.relu(out)
            #print('1  ',out.shape)
            '''end layer sigmoid is essential as this is a classification problem'''
            fx[i] = torch.argmax(out,dim=1)
            #print('2 ',out.shape)
            out = self.softmax(out)
            #print('3  ',out.shape)
            #f()
            prob[i] = copy.deepcopy(out.detach())
        return copy.deepcopy(fx),copy.deepcopy(prob)

    #fx, prob = rnn.evaluate_proposal(data,w)
    def evaluate_proposal(self,data,w=None):
        if w is None:
            y_pred,prob = self.forward(data)
            return y_pred,prob 
        else:
...
class ptReplica(multiprocessing.Process):
    ...
    def accuracy(self,pred,actual):
        ...
        '''
    def likelihood_func(self, rnn, data, w):
        ...
        fx, prob = rnn.evaluate_proposal(data,w)
        rmse = self.rmse(fx,y)
        z = np.zeros((len(data),self.topology[2]))
        lhood = 0
...
#        y_train = self.traindata[:,netw[0]]
        ...
        #Evaluate Proposals
        pred_train, prob_train = rnn.evaluate_proposal(train,w=w)
        pred_test, prob_test = rnn.evaluate_proposal(test, w=w)
        ...
        print(time.time()-t1, ' time elapsed in sec')
        ...
        for i in range(1,samples):  # Begin sampling --------------------------------------------------------------------------
            ...
            if (self.use_langevin_gradients is True) and (lx< self.l_prob):
                w_gd = rnn.langevin_gradient(train, copy.deepcopy(w)) # Eq 8
                w_proposal = rnn.addnoiseandcopy(w_gd,0,step_w) #np.random.normal(w_gd, step_w, w_size) # Eq 7
                w_prop_gd = rnn.langevin_gradient(train, copy.deepcopy(w_proposal)) 
            ...
            [likelihood_proposal, pred_train, rmsetrain] = self.likelihood_func(rnn, train, w_proposal)

            [likelihood_ignore, pred_test, rmsetest] = self.likelihood_func(rnn, test, w_proposal)
            ...
            if u < mh_prob:
                ...
                #acc_train[i,] = self.accuracy(pred_train, y_train)  
                #acc_test[i,] = self.accuracy(pred_test, y_test)
