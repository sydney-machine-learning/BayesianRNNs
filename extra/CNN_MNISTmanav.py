...
parser.add_argument('-n', '--net', help='Choose rnn net, "1" for RNN, "2" for GRU, "3" for LSTM', default=1, dest="net",
                    type=int)
...
parser.add_argument('-t', '--temperature', help='Demoninator to determine Max Temperature of chains (MT=no.chains*t) ',
                    default=3, dest="mt_val", type=int)
...
parser.add_argument('-lr', '--learn', help='learn rate for langevin gradient', dest="learn_rate", default=0.1,
                    type=float)
...
class Model(nn.Module):
    ...
    def __init__(self, topo, lrate, rnn_net='RNN'):
        ...
        if rnn_net== 'CNN':
            ...
            if rnn_net == 'LSTM':
                self.hidden = (torch.ones((self.n_layers, self.batch_size, self.hidden_dim)),
                               torch.ones((self.n_layers, self.batch_size, self.hidden_dim)))
    ...
    def evaluate_proposal(self, data, w=None):
        ...
        #print("HI")
        y_pred = torch.zeros((len(data), 50))
        prob = torch.zeros((len(data), 50, 10))
    ...
    def langevin_gradient(self, x, w):
        # print(w)
        self.loadparameters(w)
...
class ptReplica(multiprocessing.Process):
    ...
    def __init__(self, use_langevin_gradients, learn_rate, w, minlim_param, maxlim_param, samples, traind,
                 testd, topology, burn_in, temperature, swap_interval, langevin_prob, path, parameter_queue,
                 main_process, event, rnn_net):
    ...
    def likelihood_func(self, rnn, data, w, tau_sq):
        y = torch.zeros((len(data), 50))
    ...
    def prior_likelihood(self, sigma_squared, w_list, nu_1, nu_2, w, tausq, rnn):
    ...
    def run(self):
        ...
        #pred_train,pp1 = rnn.evaluate_proposal(self.traindata, w)  #

        #pred_test,pp2 = rnn.evaluate_proposal(self.testdata, w)  #
        ...
        prior_current = self.prior_likelihood(sigma_squared, rnn.getparameters(w), nu_1, nu_2, w, tau_pro, rnn)  # takes care of the gradients
        [likelihood, pred_train, rmsetrain] = self.likelihood_func(rnn, self.traindata, w, tau_pro)
        [_, pred_test, rmsetest] = self.likelihood_func(rnn, self.testdata, w, tau_pro)
        ...
        for i in range(
            ...
            if (self.use_langevin_gradients is True) and (lx < self.l_prob):
                w_gd = rnn.langevin_gradient(self.traindata, copy.deepcopy(w))  # Eq 8
                w_proposal = rnn.addnoiseandcopy(w_gd, 0, step_w)  # np.random.normal(w_gd, step_w, w_size) # Eq 7
                w_prop_gd = rnn.langevin_gradient(self.traindata, copy.deepcopy(w_proposal))
            ...
            [likelihood_proposal, pred_train, rmsetrain] = self.likelihood_func(rnn, self.traindata,
                                                                                w_proposal, tau_pro)
            [_, pred_test, rmsetest] = self.likelihood_func(rnn, self.testdata, w_proposal, tau_pro)
            prior_prop = self.prior_likelihood(sigma_squared, rnn.getparameters(w), nu_1, nu_2, w_proposal, tau_pro,
                                               rnn)  # takes care of the gradients
            ...
            if ((i + 1) % self.swap_interval == 0 and i != 0):
                param = np.concatenate([rnn.getparameters(w).reshape(-1), np.asarray([eta]).reshape(1),
                                        np.asarray([likelihood * self.temperature]), np.asarray([self.temperature])])
        ...
        param = np.concatenate(
            [rnn.getparameters(w).reshape(-1), np.asarray([eta]).reshape(1), np.asarray([likelihood]),
...
class ParallelTempering:
    ...
    def __init__(self, use_langevin_gradients, learn_rate, traindd, testdd, topology, num_chains,
                 maxtemp, NumSample, swap_interval, langevin_prob, path, rnn_net='RNN'):
        # FNN Chain variables
        self.traindata = traindd
        self.testdata = testdd
        #self.train_y = train_y
        #self.test_x = test_x
        #self.test_y = test_y
        self.topology = topology
        self.rnn = Model(self.topology, learn_rate, rnn_net=rnn_net)
        self.num_param = sum(p.numel() for p in
                             self.rnn.parameters())  # (topology[0] * topology[1]) + (topology[1] * topology[2]) + topology[1] + topology[2] + (topology[1] * topology[1])
        ...
        for i in range(0, self.num_chains):
            ...
            self.chains.append(
                ptReplica(self.use_langevin_gradients, self.learn_rate, w, self.minlim_param, self.maxlim_param,
                          self.NumSamples, self.traindata, self.testdata, self.topology,
                          self.burn_in, self.temperatures[i], self.swap_interval, self.langevin_prob, self.path,
                          self.parameter_queue[i], self.wait_chain[i], self.event[i], self.rnn_net))
    ...
    def show_results(self):
        ...
        #fx_train = fx_train_all.transpose(2, 0, 1).reshape(self.train_x.shape[0],
                                                           #-1) # need to comment this if need to save memory
...
def main():
    ...
    for net in networks:
        for j in range(4, 5):
            ...
            if problem == 13:
                name = "MNIST"
                traind=data_load(data='train')
                testd=data_load(data='test')
                Input=1600
                Output=10
            ...
            pt = ParallelTempering(use_langevin_gradients, learn_rate, traind, testd, topology,
                                   num_chains, maxtemp, NumSample, swap_interval, langevin_prob, path, rnn_net=net)
            ...
            allres = np.asarray(
                [problem, NumSample, maxtemp, swap_interval, langevin_prob, learn_rate, rmse_tr, rmsetr_std, rmsetr_max,
                 rmse_tes, rmsetest_std, rmsetes_max, swap_perc, accept_per, timetotal])
